  ...
  return (
    <a
      ...
      { !isLoading ? 
        ...
        :
        <div loading={isLoading.toString()} className="width-40-percent min-width-200-px border-radius-4-px height-192-px"/>
      }
      <div id="content" className={
        ...
        `}>
        <h1 className="width-100-percent text-transform-capitalize font-weight-500" loading={isLoading.toString()}>
          {sanitizedProjectName}
        </h1>
        <p className="font-size-1-dot-1-rem line-height-30-px" loading={isLoading.toString()}>{description}</p>
        <div className="font-size-1-dot-1-rem display-flex gap-5-px flex-flow-wrap" loading={isLoading.toString()}>
      ...
      <div className="position-absolute display-flex gap-5-px top--13-px right-30-px">
        {!isLoading && formattedLanguagesList.map((language, index) => (
